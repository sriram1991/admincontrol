...
describe('account', () => {
  ...
  it('should fail to login with bad password', async () => {
    ...
    const value1 = await element(by.css('h1 > span')).getAttribute('sysTranslate');
    expect(value1).to.eq(expect1);
    signInPage = await navBarPage.getSignInPage();
    await signInPage.autoSignInUsing(username, 'foo');

    const expect2 = 'login.messages.error.authentication';
    const value2 = await element(by.css('.alert-danger')).getAttribute('sysTranslate');
  ...
  it('should login successfully with admin account', async () => {
    ...
    const value1 = await element(by.className('username-label')).getAttribute('sysTranslate');
    expect(value1).to.eq(expect1);
    await signInPage.autoSignInUsing(username, password);

    const expect2 = 'home.logged.message';
    await browser.wait(ec.visibilityOf(element(by.id('home-logged-message'))));
    const value2 = await element(by.id('home-logged-message')).getAttribute('sysTranslate');
    expect(value2).to.eq(expect2);
  });
  ...
  it('should be able to update settings', async () => {
    ...
    const value2 = await alert.getAttribute('sysTranslate');
    expect(value2).to.eq(expect2);
  });
  ...
  it('should be able to update password', async () => {
    ...
    const alert = element(by.css('.alert-success'));
    const alertValue = await alert.getAttribute('sysTranslate');
    ...
    const alertValue2 = await alert.getAttribute('sysTranslate');
